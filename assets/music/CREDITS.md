# Müzik Telif & Kaynak Kayıtları

Tüm kayıtlar archive.org'dan indirildi (2026-07-11). Lisanslar doğrulandı.

| Dosya | Parça | Sanatçı/Kayıt | Lisans | Atıf gerekli mi? |
|-------|-------|---------------|--------|------------------|
| `clair-de-lune.mp3` | Debussy — Clair de Lune | **Laurens Goedhart (2011, Wikimedia Commons)** | CC BY 3.0 | **EVET — footer'da verildi** |
| ~~arabesque~~ | ~~Debussy — Arabesque (Sassoli, tarihi)~~ | ~~emekli — cızırtı~~ | PD | — |
| `sarabande.mp3` | Handel — Sarabande | zero-project | CC BY 3.0 | **EVET** |
| `chopin-nocturne.mp3` | Chopin — Nocturne Op.55 No.1 | parso (CC0 modern kayıt) | CC0 1.0 | Hayır (kamu malı) |

## ⚠️ Handel — Sarabande için ATIF ZORUNLU (CC BY 3.0)
Bu kayıt ticari sitede kullanılabilir AMA görünür bir yerde şu kredi bulunmalı:
> "Sarabande (G.F. Handel) — performans: zero-project, CC BY 3.0"

Öneri: sitenin footer'ına küçük bir "Müzik kredileri" satırı ya da Gün 1 sayfasında dipnot.
Kredi vermek istemezsen bu parçayı kamu malı bir Sarabande kaydıyla değiştirebiliriz
(şu an archive.org'da temiz PD Handel Sarabande bulunamadı — alternatif besteci/parça gerekebilir).

## Kaynaklar
- Debussy: archive.org/details/debussy-first-arabesque-ada-sassoli
- Handel: archive.org/details/jamendo-331013
- Chopin: archive.org/details/a-5800-godowsky-chopin-nocturne

## Not — kayıt kalitesi
- **Chopin (Gün 2):** İlk konan Godowsky/Op.9 No.2 kaydı 1920'lerden tarihiydi ve CIZIRTILIYDI (Hilal 11.07 bildirdi) → temiz CC0 modern parso kaydı Op.55 No.1 ile DEĞİŞTİRİLDİ. Not: tam "Op.9 No.2"nin hem temiz hem telifsiz kaydı bulunamadı; onu birebir isteyen Musopen'den (ücretsiz hesap) indirmeli.
- **Debussy (index+platform):** Ada Sassoli kaydı cızırtılıydı (Hilal 11.07 bildirdi) → ffmpeg restorasyonu yapıldı (adeclick+afftdn+bant filtresi) → `arabesque-v2.mp3`. Hâlâ yetersizse: Musopen CC0 versiyonu elle indirilecek ya da parça değişecek.
- **Handel (Gün 1):** zero-project modern kayıt, ses temiz.


## GÜNCELLEME (11.07 akşam)
- Ana sayfa + Platform: Arabesque (tarihi kayıt, restore yetmedi) → **Clair de Lune (Goedhart, CC BY 3.0, Wikimedia Commons)**. Kaynak: commons.wikimedia.org 'Clair de lune (Claude Debussy) Suite bergamasque.ogg'
- CC-BY yükümlülüğü kapatıldı: 4 sayfanın footer'ına müzik kredisi satırı eklendi (Debussy + Handel + Chopin).
- music.js v4: fade hızlandırıldı (geç başlama şikayeti), baştaki sessizlik mp3'te kırpıldı.

## GÜNCELLEME 2 (11.07 gece)
- **Platform sayfası (UP-HB Üretken HUB):** Clair de Lune → **Bach — Cello Suite No.1 Prelüd (Kevin MacLeod, incompetech.com, CC BY 3.0)** — Hilal'in isteği. Kaynak: Wikimedia Commons "Cello Suite -1 in G - Prelude (ISRC USUAN1100298).mp3". Atıf footer kredi satırına eklendi (4 sayfa).
- Ana sayfa Clair de Lune'de kaldı.

## GÜNCELLEME 3 (11.07 gece)
- **Gün 2 (Vibe Coding):** Chopin Nocturne → **Beethoven — Ay Işığı Sonatı 1. bölüm (Adagio sostenuto), Paul Pitman, KAMU MALI (Musopen kaydı)** — Hilal'in isteği. Kaynak: Wikimedia Commons. Atıf gerekmez; footer'da yine de anıldı.
- Chopin (chopin-nocturne.mp3) repertuvardan çıktı, dosya arşivde duruyor.
